...
class Register_model extends CI_Model {
	...
	public function __construct() 
	{
		...
		public function add_registered()
		{
			$set_data = array
			(
			'fname'=> mysqli_real_escape_string($this->get_mysqli(),ucwords($this->input->post('rfname'))),
			'lname'=> mysqli_real_escape_string($this->get_mysqli(),ucwords($this->input->post('rlname'))),
			'email'=> mysqli_real_escape_string($this->get_mysqli(),$this->input->post('remail')),
			'mobile'=> mysqli_real_escape_string($this->get_mysqli(),$this->input->post('rmobile')),
			'password' => mysqli_real_escape_string($this->get_mysqli(),$this->input->post('rpassword')),
			/*'checkbox' => mysqli_real_escape_string($this->get_mysqli(),$this->input->post('checkbox')),*/
		...
		public function edit_registered()
		{
			...
			(
				...
				'zip'      => mysqli_real_escape_string($this->get_mysqli(),$this->input->post('zipcode',TRUE)),
				'create_date_time' => date('Y-m-d H:i:s'),
				'status' => '1'
			);
			$this->db->where('id',$this->session->userdata('user_id'));
